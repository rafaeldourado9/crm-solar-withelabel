  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-primary text-white h-screen p-6 flex flex-col fixed left-0 top-0 overflow-y-auto z-50 transition-transform duration-300 shadow-2xl ${
        ...
        <div className="mb-8">
          {branding.logo_url ? (
            <img src={branding.logo_url} alt={branding.nome_fantasia} className="h-10 object-contain mb-2" />
          ) : (
            <h1 className="text-2xl font-bold">
              {branding.nome_fantasia.split(' ')[0]}{' '}
              <span className="text-accent">{branding.nome_fantasia.split(' ').slice(1).join(' ') || 'Solar'}</span>
            </h1>
          )}
